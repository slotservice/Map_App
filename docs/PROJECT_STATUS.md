...
* `2026-04-30` — Andrew — Phase-3 cutover infrastructure landed. **Migration ETL fully implemented** (was stubs): legacy MariaDB connector with typed row interfaces, target Prisma client wrapper, R2 uploader with HEAD-then-PUT dedup + p-limit concurrency, magic-byte image format sniffer (PNG / JPG / WEBP / HEIC). All 7 transforms now real: users (preserves $2y$ bcrypt, splits username, role mapping 1/2/4 → admin/vendor/worker, fallback viewer for unknown), maps (derives task_columns + count_columns from union of tasks.data keys, drops empty-key bug, folds tagemails into tag_alert_recipients), stores (legacy tasks → stores + store_tasks, zip coerced to text fixing the leading-zero loss, lat/lon validated, raw passthrough preserved, deletes+rebuilds StoreTask rows for idempotency), photos (walks LEGACY_PHOTOS_DIR, sniffs format, SHA-256 dedup, deterministic Photo.id from legacyPath so re-runs are idempotent), completions (dedupes by task_id keeping most recent, parses images JSON to relink before/after photos via legacyPath, builds completion_counts from countColumns ∩ tasks.data, points signature photo), tag-alerts (legacy missingtag → tag_alerts + tag_alert_photos with tag_alert kind set, emailStatus='sent' historical), assignments (dedup, role from migrated user). New `verify.ts` post-migration row-count diff with tolerance windows + spot-check. **Deployment configs** for production: `railway.json` for the API, `apps/admin/vercel.json` for the admin web, comprehensive `docs/DEPLOY.md` runbook covering R2 + Postmark + Railway + Vercel + DNS + EAS mobile builds + cutover steps. **Smoke-test script** (`infra/smoke-test.sh`) — curl-based, no GUI required, checks /healthz + /readyz + login + 401-on-anon + map list + OpenAPI docs reachability. **Handover documents:** `docs/HANDOVER.md` (final-delivery checklist + ops basics + bug-fix-window scope), `docs/TRAINING.md` (30-min handover-call running order), `docs/CREDENTIALS_TEMPLATE.md` (template for the credentials hand-over PDF, never commit filled-in version). **Daily updates pre-drafted:** `DAILY_UPDATES.md` (kept locally, never committed) holds Days 2-7 update messages for the user to copy-paste into Freelancer chat each morning, plus skeleton headlines for Days 8-17.
* `2026-04-30` — Andrew — Test coverage + push-notifications scaffold + auth-refresh polish. **Tests:** extracted the marker-colour state machine to `@map-app/shared/marker.ts` as a pure function (StoresService now delegates to it); 7-case unit-test suite covering blue / yellow / orange / red transitions including the subtle "imported as already-complete shouldn't be red" edge. Photo format-detection tests for PNG/JPG/WEBP/HEIC + non-image fallback (RIFF-but-not-WEBP, short buffer, PDF). 4-case timezone-formatter test against three real IANA tzs proving legacy bug L5 ("military-time PM") can't recur. 9-case DTO schema test covering negative paths (empty old password, weak new password, photoIds cap, MB cap, recipient cap, admin-not-assignable). **Push notifications scaffold (Phase-4 prep):** new `device_tokens` Prisma model + `register/deregister` endpoints (`POST /devices`, `DELETE /devices/:pushToken`); `ExpoPushHandler` in the outbox dispatches via Expo's free HTTP push API (works on Android + iOS without per-platform certificate juggling); first wired trigger fires when an admin assigns a worker to a map → "New map assigned" notification with a deep-link payload. Mobile registers the Expo push token on login + hydrate (`registerForPush` requests permission, sets the Android channel, posts to /devices), deregisters on logout. **Polish:** mobile + admin API clients now do single-flight refresh-token rotation on 401 (closes a real bug — without this, every worker session would silently break after 15 min). Admin client also redirects to /login when refresh fails. New `friendlyError` helper translates RFC-7807 ApiError envelopes to human messages (e.g. INVALID_CREDENTIALS → "Wrong email or password"); login page now uses it instead of the raw error message.
* `2026-05-01` — Andrew — Deploy hardening (`2263e6a`). Committed the missing Prisma init migration (`20260430022723_init`, all 16 tables + 7 enums) — was untracked on Tomas, would have broken any fresh `prisma migrate deploy`. Also bound Postgres + Mailhog SMTP to `127.0.0.1` in `infra/docker-compose.yml` (dev DB + open SMTP relay would otherwise be publicly reachable on a shared-IP host). NOT committed: `pnpm-workspace.yaml`'s narrowing of `apps/*` → `apps/api+apps/admin` — that's a workspace-wide regression for a VPS-local install issue (Expo native deps); right answer is `--filter` at install time. Installed Mailhog binary (`/usr/local/bin/mailhog` v1.0.1) under PM2 (`mailhog` process), bound localhost-only on 1025 (SMTP) + 8025 (UI). Verified the full async email pipeline end-to-end: `POST /auth/forgot-password` → outbox row → 5s poll → `PasswordResetEmailHandler` → SMTP → Mailhog inbox shows "Reset your Full Circle FM password" addressed to admin@fullcirclefm.local. Tag-alert emails will work the same way. UI is reachable via `ssh -L 8025:localhost:8025 Tomas` (not exposed publicly).
* `2026-04-30` — Andrew — **First end-to-end deploy on Tomas VPS** (`3c78a79`). Stack now live at `http://80.241.222.224:3001` (API) + `http://80.241.222.224:4000` (admin web). Both processes managed by PM2 (`map-api`, `map-admin`), persisted via `pm2 save`. Postgres 17 native (`mapapp_dev` DB, port 5432 bound to 127.0.0.1, all 16 Prisma migrations applied), MinIO native (`fcfm-photos-dev` bucket, port 9000), no Docker per the project's PM2-over-Docker preference. Seed users present: `admin@fullcirclefm.local` / `worker@…` / `vendor@…`, password `password123`. Smoke test `infra/smoke-test.sh` passes 10/10 (healthz, readyz with DB ping, admin login + role check, authenticated /maps + /users, 401 on bogus password, 401 on anon /maps, OpenAPI docs reachable). Public-IP login flow verified end-to-end (token issued, /maps returns 200). **Three admin build blockers fixed** along the way (latent because the sprint never ran the prod build): `roles.includes(user.role)` rejected `"worker"` because the array literal narrowed away the unused enum member (fixed via explicit `NavItem` interface); Next 14's `next lint` is incompatible with eslint 9 (gated out of build with `eslint.ignoreDuringBuilds = true`, lint still runs in CI); admin `start` script's hardcoded `-p 3000` blocked PM2's `PORT=4000` override and conflicted with an unrelated cregen-front process running on 3000. **Mailhog NOT running** (was Docker-only in the dev stack); tag-alert and password-reset emails will queue and exhaust their 5-attempt backoff. Acceptable for read-only demo; needs the mailhog binary or a real SMTP relay before any flow that sends mail is exercised.
* `2026-04-30` — Andrew — Build hygiene fix (`ff05419`). The Phase-1/2 sprint shipped 15 latent TypeScript errors across 7 files because `pnpm build` was never run — only `nest start --watch` (more permissive) and vitest (which doesn't fully typecheck non-test imports). All errors were typing-only against Prisma generated types: missing `@types/multer` (Express.Multer.File), `Record<string, unknown>` not satisfying `Prisma.InputJsonValue` on Json writes (audit payload, store.raw, outbox payload), `JsonValue` reads in 3 outbox handlers requiring cast through `unknown`, exceljs's pre-generic `Buffer` type colliding with node-22's `Buffer<T>`, and a too-loose `ParsedRow.fixed` shape. Zero runtime behaviour change. Verified on Tomas VPS: `pnpm --filter @map-app/api build` exits 0; all 28 unit tests still pass. **Side findings (NOT fixed yet):** (a) `package.json:start` says `node dist/main.js` but tsc emits to `dist/src/main.js` because `tsconfig.rootDir = "."` — will block production start; (b) `pnpm-lock.yaml` is untracked, allowing local↔VPS dependency drift; (c) CI workflow does not run `pnpm -r build`, hence the latent errors slipped through.
